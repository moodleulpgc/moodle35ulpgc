...
class enrol_waitlist_edit_form extends moodleform {
    ...
    function definition() {
        ...
        /* ecastro ULPGC, removed because useless at ULPGC. fields used for cohorts
        ...
        */
        ...
        // Faculty & users allowed to use this
        $mform->addElement('header', 'headerallowedusers', get_string('headerallowedusers', 'enrol_waitlist'));

        $options = $this->get_multicohort_options($instance, $context);
        $size = count($options);
        if($size >= 8) {
            $size = 8;
        }
        
        $select = $mform->addElement('select', 'customchar3', get_string('oranycohorts', 'enrol_waitlist'), $options);
        $mform->addHelpButton('customchar3', 'oranycohorts', 'enrol_waitlist');
        $select->setMultiple(true);
        $select->setSize($size);
        
        $options = array('' => get_string('none')) + $options;
        $select = $mform->addElement('select', 'customchar1', get_string('andallcohorts', 'enrol_waitlist'), $options);
        $mform->addHelpButton('customchar1', 'andallcohorts', 'enrol_waitlist');
        $select->setMultiple(false);
        $select->setSize($size);
      
        $select = $mform->addElement('select', 'customchar2', get_string('notcohorts', 'enrol_waitlist'), $options);
        $mform->addHelpButton('customchar2', 'notcohorts', 'enrol_waitlist');
        $select->setMultiple(false);
        $select->setSize($size);

        $mform->addElement('static', 'explain1', '', get_string('noenrolgroups', 'enrol_waitlist'));
        $mform->addElement('advcheckbox', 'customint7', get_string('assigngroup', 'enrol_waitlist'));
        $mform->addHelpButton('customint7', 'assigngroup', 'enrol_waitlist');
       
        $groups = $this->get_group_options($context);
        $mform->addElement('select', 'customint8', get_string('enrolgroup', 'enrol_waitlist'), $groups);
        $mform->addHelpButton('customint8', 'enrolgroup', 'enrol_waitlist');
        ...
        if(count($fields) > 0){
            ...
            $mform->addElement('header', 'headercustomfields', 'Custom Fields');
        ...
        $instance->customchar3 = !empty($instance->customchar3) ? explode(',', $instance->customchar3) : array();
    ...
    protected function get_multicohort_options($instance, $context) {
        ...
        $multicohorts = array();
        if ($instance->id < 0) {
            $used = $instance->customchar3;
            if($instance->customchar1) {
                $used .= ','.$instance->customchar1;
            }
            ...
            $allmulticohorts = $DB->get_records_select('cohort', "idnumber $insql", $params, 
        ...
        foreach ($allmulticohorts as $c) {
            $multicohorts[$c->idnumber] = format_string($c->name);
        }
    ...
    protected function get_group_options($coursecontext) {
        ...
        //$groups[MULTICOHORT_KEEP_GROUP] = get_string('keepgroups', 'enrol_waitlist');
        if (has_capability('moodle/course:managegroups', $coursecontext)) {
            //$groups[WAITLIST_CREATE_GROUP] = get_string('creategroup', 'enrol_waitlist');
            $groups[WAITLIST_MULTIPLE_GROUP] = get_string('multiplegroup', 'enrol_waitlist');
        }
