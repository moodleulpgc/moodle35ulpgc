...
if (!$controller->is_form_defined() || empty($options['alwaysshowdefinition'])) {
    throw new moodle_exception('nopermissions', 'error', '', get_string('previewerubric', 'gradingform_erubric'));
}

$title = get_string('gradingof', 'gradingform_erubric', $manager->get_area_title());
$PAGE->set_url(new moodle_url('/grade/grading/form/erubric/preview.php', array('areaid' => $areaid)));
