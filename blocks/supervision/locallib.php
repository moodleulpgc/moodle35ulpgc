 ...
 */
    ...
    function supervision_assign_supervisor($data, $delete = false) {
        ...
        $success = false;
        $items = array();
        $supervisor_role = get_config('block_supervision', 'checkerrole');
        ...
        if($DB->record_exists('role', array('id'=>$supervisor_role))) {
            ...
            foreach($items as $item) {
                if($data->review AND !$delete) {
                    role_assign($supervisor_role, $data->userid, $item, 'block_supervision' );
                } elseif(!$data->review OR $delete)  {
                    role_unassign($supervisor_role, $data->userid, $item, 'block_supervision' );
                }
                $success = true;
            }
        }
    ...
    function supervision_warnings_mailing($timetocheck, $lasttime) {
        ...
        if ($rs = $DB->get_recordset_sql($sql, $params)) {
            $from = get_string('warningautomatic',  'block_supervision');
            $supportuser = '';
            ...
            foreach ($rs as $stat) {
                ...
                if($stat->studentid) {
                    ...
                    $info->student = get_string('emailstudent',  'block_supervision', $st );
                }

                $subject = get_string('warningmailsubject', 'block_supervision', $stat->shortname);
                $text = get_string('warningemailtxt',  'block_supervision', $info );
                $html = ($stat->mailformat == 1) ? get_string('warningemailhtml',  'block_supervision', $info ) : '';
    ...
    function supervision_ulpgc_dept_supervisors() {
        ...
        $ulpgc_supervisors = array();
        if(!get_config('block_supervision', 'enabledepartments')) {
    ...
    function supervision_ulpgc_faculty_supervisors() {
        ...
        $ulpgc_supervisors = array();
        if(!get_config('block_supervision', 'enablecategories')) {
            return $ulpgc_supervisors;
        }
        $sql = "SELECT c.id, ud.id AS director, us.id AS secretario
                    FROM {centros} c
