...
class core_course_management_renderer extends plugin_renderer_base {
    ...
    public function category_bulk_actions(coursecat $category = null) {
        // Resort courses.
        // Change parent.
        if (!coursecat::can_resort_any() && !coursecat::can_change_parent_any() && ($category && !$category->can_resort_courses())) { // ecastro ULPGC allow courses resort in non-editable category
            return '';
        }
        ...
        if (coursecat::can_resort_any() || ($category && $category->can_resort_courses())) {
            $selectoptions = array(
                'selectedcategories' => get_string('selectedcategories'),
                'allcategories' => get_string('allcategories')
            );
            if(!$canchange = coursecat::can_resort_any()) { // ecastro ULPGC allow courses resort in non-editable category
                $selectoptions = array();
