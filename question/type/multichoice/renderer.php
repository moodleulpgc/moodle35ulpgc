...
abstract class qtype_multichoice_renderer_base extends qtype_with_combined_feedback_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa,
        ...
        // ecastro ULPGC to allow unmarking
        $result .= html_writer::start_tag('div', array('class' => 'answer unmark'));
            $respid =  $this->get_input_id($qa, '');
            $onclick = " blankresponse('$respid'); ";
            $result .= html_writer::empty_tag('input', array('type'=>'button', 'value'=>get_string('noresponse', 'quiz'), 'onClick'=>$onclick));
            $js = "function blankresponse(name) {
                        var resps = document.getElementsByName(name);
                        for (var i=0;i<resps.length;i++) {
                            resps[i].checked=false;
                        }
                    }";
            $result .= html_writer::script($js);
        $result .= html_writer::end_tag('div'); // unmark
    ...
    public function formulation_export(question_attempt $qa, question_display_options $options) { // ecastro ULPGC
        ...
        foreach($choices as $choice) {
            $choicestext .= $choice.'<br />' ; //html_writer::tag('div', $choice);
        }
        */
