...
class qtype_recordrtc_renderer extends qtype_renderer {
    ...
    public function formulation_and_controls(question_attempt $qa, question_display_options $options) {
        ...
        // ecastro ULPGC
        $mediafilename = ($question->mediatype == 'video') ? qtype_recordrtc::VIDEO_FILENAME : qtype_recordrtc::AUDIO_FILENAME;
        
        foreach ($existingresponsefiles as $file) {
            //if ($file->get_filename() === qtype_recordrtc::AUDIO_FILENAME) {
            if ($file->get_filename() === $mediafilename) {
        ...
        if ($options->readonly) {
            ...
            if ($existingresponsefile) {
                ...
                $recordingurl = moodle_url::make_draftfile_url($draftitemid, '/', $mediafilename);
                $state = 'recorded';
