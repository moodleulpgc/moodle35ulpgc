...
if (defined('USE_GET_SHORTCUTS')) {
    ...
    function game_get_shortcuts($defaultitem) {
        ...
        if(get_config('local_ulpgccore', 'enabledadvchooser')) {  // ecastro ULPGC shorthen shortcuts, too long on moodchooser
            $prefix = strlen(get_string('pluginname', 'game').' - ');
            foreach($types as $type) {
                 $type->title = trim(substr($type->title, $prefix)); 
            }
        }
