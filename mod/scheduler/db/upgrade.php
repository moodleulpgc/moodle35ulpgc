...
function xmldb_scheduler_upgrade($oldversion=0) {
    ...
    if ($oldversion < 2015062601) {
        ...
        $oldfield = new xmldb_field('forcegroupappoint', XMLDB_TYPE_INTEGER, '4', null, XMLDB_NOTNULL, null, '0'); // ecastro ULPGC 
        if ($dbman->field_exists($table, $oldfield)) { // ecastro ULPGC custom use of bookingrouping
            $DB->set_field('scheduler', 'bookingrouping', -1, array('forcegroupappoint'=>0));
            $DB->set_field('scheduler', 'bookingrouping', 0, array('forcegroupappoint'=>1));
            $DB->set_field('scheduler', 'bookingrouping', 0, array('forcegroupappoint'=>2));
        }
        
/*
        // Convert old group mode into instance setting for scheduler. // removed ecastro ULPGC    
...
*/ 
        ...
        $field = new xmldb_field('usesharedslots', XMLDB_TYPE_INTEGER, '2', null, XMLDB_NOTNULL, null, '0', 'bookingrouping'); // ecastro ULPGC
        ...
        if (!$dbman->field_exists($table, $field)) {
         ...
         // Conditionally launch add field bookingrouping.
        if ($dbman->field_exists($table, $field)) {
            $sql = "SELECT id, schedulerid 
                    FROM {scheduler_slots} 
                    WHERE shared > 0 
                    GROUP BY schedulerid ";
            if($schedulers = $DB->get_records_sql_menu($sql, null)) {
                $chunks = array_chunk($schedulers, 500);
                foreach($chunks as $chunk) {
                    $insql = "= ''";
                    $inparams = array();
                    list($insql, $inparams) = $DB->get_in_or_equal($chunk);
                    $DB->set_field_select('scheduler', 'usesharedslots', 1, " id $insql ", $inparams);
                }
            }
