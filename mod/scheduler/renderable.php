...
class scheduler_slot_table implements renderable {
    ...
    public function add_slot(scheduler_slot $slotmodel, scheduler_appointment $appointmentmodel, $otherstudents, $cancancel = false) {
        ...
        // ecastro ULPGC
        $slot->appointmentnoteedit =  isset($slotmodel->data->appointmentnoteedit) ? $slotmodel->data->appointmentnoteedit : 0;
        $slot->appointmentnote = $slotmodel->appointmentnote;
        $slot->groupid = $appointmentmodel->groupid;
        $slot->groupname = $appointmentmodel->groupid ? $appointmentmodel->get_group_name() : '';
...
class scheduler_student_list implements renderable {
    ...
    public $groups = array(); // ecastro
    public $slotid = 0; // ecastro
    ...
    // ecastro ULPGC
        $student->groupid = $appointmentmodel->groupid; // ecastro ULPGC
        //$student->etutor = $appointmentmodel->etutor; // ecastro ULPGC
        $student->appointmentnote = $appointmentmodel->appointmentnote;
    ...
    public function __construct(scheduler_instance $scheduler, $showgrades = true) {
        $this->scheduler = $scheduler;
        $this->showgrades = $showgrades;
        $this->groups = array(); // ecastro ULPGC
    }
...
class scheduler_slot_booker implements renderable {
    ...
    public function add_slot(scheduler_slot $slotmodel, $canbook, $bookedbyme, $groupinfo, $otherstudents) {
        ...
        $slot->teacher = $slotmodel->get_teacher();
        $slot->otherstudents = $otherstudents;
        $slot->appointmentnote = $slotmodel->appointmentnote; // ecastro ULPGC
