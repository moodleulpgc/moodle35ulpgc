...
function dialogue_supports($feature) {
    switch($feature) {
        case FEATURE_GROUPS:                  return true; // ecastro ULPGC reimplement groups
        case FEATURE_GROUPINGS:               return true; // ecastro ULPGC reimplement groupings
        ...
        foreach ($rs as $record) {
            ...
            try {
                ...
                if($DB->record_exists_sql($sql, $params)) {
                    ...
                    $success = $reply->send(\mod_dialogue\dialogue::FLAG_SENT);
                } else {
                    $conversation->add_participant($record->userid);
                    $success = $conversation->send(\mod_dialogue\dialogue::FLAG_SENT);
                }
