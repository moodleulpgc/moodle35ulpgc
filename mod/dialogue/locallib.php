...
function dialogue_search_potentials(\mod_dialogue\dialogue $dialogue, $query = '') {
    ...
    $alternatemode      = $dialogue->activityrecord->alternatemode; // ecastro ULPGC
    $multiplemode       = $dialogue->activityrecord->multipleconversations; // ecastro ULPGC
    ...
    list($esql, $eparams) = get_enrolled_sql($context, $receivingcap, null, true); // ecastro to allow alternate opening teacheres/students
...
*/
    if ($usecoursegroups) { // ecastro ULPGC
    ...
    if(($multiplemode == 0) && !has_capability('mod/dialogue:openasstaff', $context)) { // ecastro ULPGC tu enforce single conversation by user
        $sql = "SELECT dp2.id, dp2.userid
                ...
                AND NOT EXISTS( SELECT 1 FROM {dialogue_messages} dm 
                    ...
                    ";
        if($others = $DB->get_records_sql_menu($sql, array('dialogue' =>$dialogue->activityrecord->id  , 'user'=>$USER->id))) {
...
/**
 * Get a users total UNREPLIED messages count for a dialogue course module.
 *
 * @global stdClass $USER
 * @global stdClass $DB
 * @param stadClass $cm object
 * @return int
 */
...
function dialogue_get_humanfriendly_dates($epoch) {
    ...
    */
    ...
    $customdatetime['time'] = userdate($epoch, get_string('strftimetime', 'langconfig')); //  date("g:i a", $epoch);
...
/**
* Returns eitht id or object for penultimate message in conversation by other autho not the very last
*
