...
function offlinequiz_create_pdf_question(question_usage_by_activity $templateusage, $offlinequiz, $group,
    ...
    $title .= ",  ".offlinequiz_str_html_pdf(get_string('group', 'offlinequiz')." $groupletter"); // ecastro ULPGC
    $pdf->set_title($title);
    ...
    if (!$correction) {
        ...
        $pdf->Ln(10);
        if ($offlinequiz->printstudycodefield) {
            $course = $DB->get_record('course', array('id'=>$courseid), 'id, fullname, shortname, idnumber');
            $pdf->Cell(58, 10, offlinequiz_str_html_pdf(get_string('studycode', 'offlinequiz')) . ":", 0, 0, 'R');
            $pdf->Cell(58, 10, offlinequiz_str_html_pdf(get_string('studycodecourse', 'offlinequiz', $course)), 0, 0, 'L'); // ecastro ULPGC
